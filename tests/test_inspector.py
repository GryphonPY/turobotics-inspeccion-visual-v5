...
def test_inspector_closes_a_complete_cycle_from_fixture() -> None:
    inspector = V5Inspector(ROOT)
    snapshot = _snapshot(FIXTURES / "complete.png")
...
def test_inspector_rejects_a_blurred_capture_as_unreliable() -> None:
    inspector = V5Inspector(ROOT)
    snapshot = _snapshot(FIXTURES / "blurred.png")
...
def test_inspector_identifies_missing_c08_for_the_component_map() -> None:
    inspector = V5Inspector(ROOT)
    image = cv2.imread(str(FIXTURES / "missing_c08.png"))
    assert image is not None
